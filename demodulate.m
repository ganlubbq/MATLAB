...
even=x.*ec; %multiplying by carriers
oc=sin(2*pi*fc*t).*sin(pi*t/(2*Tb));
odd=x.*oc; %multiplying by carriers
j=1;

% integration over a period of 2Tb
for i=1:2*c:length(even)-2*c
    z(j)=integral(even(i:i+2*c))>0; % decision
    j=j+2;
end
j=2;
for i=c:2*c:length(even)-c
    z(j)=integral(odd(i+c:i+2*c))>0; % decision
    j=j+2;
end
%for i=1:length(z)
 %   if z(i)>0
